...
//            grupos=grupos.substr(0,grupos.length-1);
	...
	$(document).on("click",".btn-pujar",function(e){
		e.preventDefault();
		/*
		var obj=$(this).data("obj");
		$("div#detalle-publicacion span:nth(0)").text(obj.descripcion);
		$("div#detalle-publicacion span:nth(1)").text(formatearNum(obj.precioi));
		$("div#detalle-publicacion span:nth(2)").text(formatearNum(obj.preciol));
		$("div#detalle-publicacion span:nth(3)").text(formatearNum(obj.precioa));
		ofertaActual=parseFloat(obj.precioa);
		ofertaInicial=parseFloat(obj.precioi);
		idActual=parseFloat(obj.id);
		limiteActual=parseFloat(obj.preciol);
		$("div#detalle-vendedor span:nth(0)").text(obj.nombres + ' ' + obj.apellidos);
		$("div#detalle-vendedor span:nth(1)").text(obj.email);
		$("div#detalle-vendedor span:nth(2)").text(obj.telefonos);
		$("#pujar img").attr("src",obj.ruta);
		*/
		if($(this).data("id")=="new"){
			swal({
				title:"Registrarse",
				text:"No has iniciado sessión, ¿deseas crear una cuenta?",
				imageUrl:"galeria/img/logos/bill-ok.png",
				showConfirmButton:true,
			...
			function(isConfirm){
				if(isConfirm){
					...
					$.ajax({
						url:"fcn/f_index.php",
						data:{metodo:"pujar",id:idActual,limite:limite,monto:valor},
						type:"POST",
						dataType:"json",
						success:function(data){
							console.log(data);
			...
			document.location="detalle.php?id=" + $(this).data("id");
		}
	});
...
function formatearNum(amount,decimals=2){
